...
| `tests/test_graph.py` | `graph.invoke` 输入补 `"session_id"`（MemoryState 声明通道后缺键即 KeyError） | 第 6 步 |
...
测试策略先立三条原则。**其一，零 LLM**：`extract_l1`/`consolidate_l2`/`consolidate_profile` 的核心是 prompt 工程，留给手动场景验证；单测聚焦数据库操作层（db.py）的确定性行为——幂等、隔离、排序、窗口联动都是纯 Mongo 语义。**其二，连真库**：对齐 `test_api.py` 连真 MongoDB 的既有做法，不引 mock——存取往返、幂等更新这些测试的价值恰恰在验证真实的 BSON 行为。**其三，用唯一 session_id 做隔离**：每个测试用 `uuid4()` 生成自己的会话 ID，互不污染也绝不碰真实会话的数据；唯一的例外是 L5——它全局只有一条，测试前先备份、测完用同一个公开 API 恢复。

完整测试文件（八个测试对应下面清单的每一项）：
```python
...
- **`_l1` 辅助函数**：三个存取类测试都要造 L1 数据，收拢成一个工厂函数避免重复。时间戳用手工字符串而非 `datetime.now()`——**排序断言需要确定性**：ISO 8601 字符串按字典序即时间序（第 2 步的设计决定），手工递增的时间戳让「倒序」断言在任何机器、任何执行速度下都稳定。
- **`test_get_memories_for_context` 的备份-恢复**：L5 的幂等键是 `level="L5"` 全局一条——测试库里的画像就是**真实使用中的画像**。直接 `upsert_profile` 会把它覆盖掉。做法是先 `load_memories(level="L5")` 备份，`try/finally` 里测完用同一个 `upsert_profile(backup)` 恢复——全程走公开 API，测试与记忆层保持同样的分层纪律（不碰 `_memories_collection`）。`finally` 保证断言失败也恢复。
- **`test_injection_window_default` 的行为级断言**：窗口联动可以静态断言（检查函数签名默认值），但行为级断言更有力——故意攒 25 条 L1（比窗口多 5 条），断言取回恰好 20 条。「L1 淹没 L2」这个设计担忧由测试钉死：`2×L2_TRIGGER_THRESHOLD` 的派生关系若被改坏，此测试立即红。
- **幂等的断言三件套**：`id1 == id2`（返回同一 `_id`）+ `len == 1`（没有新增）+ `content == "第二版"`（更新生效）。三个断言各自拦一种坏法：返回新 id 但没写库、写了第二条、或既没新增也没更新。
- **`test_consolidate_endpoint_guard` 只测 L2 分支**：无 L1 的会话是确定性的（唯一 session_id 保证）。L5 分支的守卫条件是「全库无 L2」——依赖库的全局状态，共享真库里可能残留别处的 L2，断言会随环境翻转；它归完成检查的场景验证，与 LLM 部分同理。
- **`test_prompts_keys_and_format`**：`.format(参数集)` 是占位符的契约测试——prompt 模板改了占位符名（如 `{user}` 改 `{question}`）而调用点没跟，运行时才炸 `KeyError`；这个测试把它提前到测试期。反向的坑由「`{` not in persona」拦住：人设不需要参数，模板里混入裸花括号会在渲染时留下残迹。

L1 / L2 / L5 的 LLM 调用涉及真实模型，标记为集成测试（对齐 `test_graph.py` 打真 LLM 的做法）。L2 阈值自动触发是图内编排行为且涉及真 LLM，归入完成检查的场景验证，不进本文件单测。

本步还有一个对模块 2 测试的波及：`test_graph.py` 的 `graph.invoke` 输入需补 `"session_id": "test-graph"`——`MemoryState` 声明了通道，缺键会让 `retrieve_memory` 直接 `KeyError`（第 4 步易错点②的同一形态）。
...
8 个测试全部通过（约 6 秒——零 LLM 的代价只是 Mongo 往返）。
