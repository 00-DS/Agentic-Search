...
- **LLM 客户端共享（模块 4 文档定稿，代码待实现）**：`services/llm.py` 模块级单例——`llm`（`init_chat_model` 原始实例，供 `build_graph()` 内 `bind_tools`）与 `call_llm(prompt)`（裸调用，供记忆层）。`graph.py` 与 `memory/store.py` 共用，`bind_tools` 留在 `build_graph()` 内（工具绑定是图特有的）。`call_llm` 里外层 `str(llm.invoke(...).content)` 是 Pylance 绕过（`content` 静态类型为联合类型，同 `parse_pdf` 惯例），别删；消费方解析用 `json.loads`（吃字符串）。
- **prompt 集中管理（模块 4 文档定稿，代码待实现）**：全部 LLM 话术在 `configs/prompts.yaml`（四键：persona / l1_extract / l2_consolidate / l5_profile），`configs/prompts.py` 模块级 `PROMPTS` 单例加载。占位符走 `str.format` 约定（字面大括号双写 `{{`）。persona 由模块 4 引入：`llm_call` 每轮前置 `SystemMessage(content=PROMPTS["persona"])`（存于调用不入 state，与 `retrieve_memory` 的动态记忆注入正交）。`retrieve_memory` 的记忆格式化留在代码（消息拼装，归代码）。依赖 `pyyaml` 转正为直接依赖。
