...
**Goal:** 修订 `任务文档/04-TMT记忆系统.md` 的记忆注入叙事——从"方案 A 是将就、检索是正道"反转为"分层配额注入与业界一线 agent 同构"，实现与依赖零改动。

**Architecture:** 只改 04 一个文件的三处文字（学习目标/注入策略小节/FAQ/差异表/学习目标 6 与两处函数 docstring 中的"方案 B"提法），依据 spec `docs/superpowers/specs/2026-08-15-04-memory-narrative-industry-alignment-design.md` 的业界实证表。

**Tech Stack:** Markdown。业界证据：`can1357/oh-my-pi`、`NousResearch/hermes-agent`、`openai/codex`、`anthropics/claude-code`、`cline/cline`、`google-gemini/gemini-cli`、`TiMEM-AI/TiMEM`。
...
改为：
```
6. 理解"分层配额注入"为何是业界一线 agent 的主流记忆方案（对照 oh-my-pi 与 hermes-agent 源码），以及提取时对比历史窗口（recent_l1）从源头减少重复
```
...
本模块的注入方式是**配额制**：profile（全局唯一 1 条）+ 本会话 L1/L2 最近 N 条（≤20），
按固定预算注入上下文。这与业界一线 agent 工具的核心记忆链路同构：

| 工具 | 长期记忆机制 | 记忆召回方式 |
|---|---|---|
| **oh-my-pi**（omp） | 启动时两阶段 LLM 管线：逐会话提取 → 跨会话整合，写入 `MEMORY.md`，会话开始作为 Memory Guidance 块注入（`packages/coding-agent/src/memories/`） | memory:// 按需读取 |
| **hermes-agent** | `MEMORY.md`（agent 自记事实）+ `USER.md`（用户画像）两个纯文本文件，冻结快照注入 system prompt（`tools/memory_tool.py`） | SQLite FTS5 全文检索（`tools/session_search_tool.py`） |
| OpenAI Codex CLI | `AGENTS.md` 逐层拼接注入（`codex-rs/core/src/agents_md.rs`） | ripgrep 字面量搜索（`codex-rs/rollout/src/search.rs`） |
| Claude Code | `CLAUDE.md` 四级层级 + auto memory 目录，`MEMORY.md` 索引前 200 行启动注入 | 模型自行读取 markdown 文件 |
| Cline / Gemini CLI | `.clinerules` / `GEMINI.md` 文件注入 | 文件按需读取 / MemTool 写回 |
| TiMeM（记忆产品参照） | L1-L5 分层提炼 | 双通道检索：语义向量 ×0.9 + BM25 ×0.1（Qdrant + Qwen3-Embedding） |

两个值得注意的结构对应：**omp 的两阶段管线（逐会话提取→跨会话整合）与本模块的
`extract_l1`→`consolidate_l2` 同构；hermes 的 `USER.md` 与本模块的 L5 profile 同构**——
"LLM 分层提炼 + 文本存储 + 启动注入"是 agent 工具的通行做法，向量检索属于记忆产品
（如 TiMeM）在海量记忆场景下的可选路线。

（保留原文"**跨会话记忆由 profile 承担**：……"一段，一字未动。）
```
...
git commit -m "docs(04): 注入策略重写为业界同构叙事 + 对照表（omp/hermes 等）"
```
...
改为：
```
**将来记忆多了怎么办？** 两条路线，按产品形态选。**agent 路线（业界主流）**：压缩与全文检索——hermes 用 SQLite FTS5 全文检索跨会话对话（`tools/session_search_tool.py`），omp/claude code 在上下文逼近上限时用 LLM 摘要压缩历史，全程零向量依赖。**记忆产品路线（可选）**：TiMeM 式双通道检索——语义向量（权重 0.9）+ BM25 关键词（0.1）；即便选这条路，omp 的 mnemopi 后端也只把向量存进自有 SQLite，而非引入独立向量数据库。无论哪条，`get_memories_for_context` 的函数名都是为切换预留的接口，图编排保持原样。
```

- [ ] **Step 2: 差异表补一行（在"去重"行之前插入）**

```markdown
| 检索 | 双通道：语义向量 + BM25（Qdrant） | 配额注入（零向量依赖） | 与 oh-my-pi/hermes/Codex/Claude Code 核心记忆链路同构；向量检索属于记忆产品的可选插件层 |
```
...
Run: `grep -n "不使用\|不采用\|不用" "任务文档/04-TMT记忆系统.md"`
Expected: 无输出。
Run: `grep -c "零向量依赖\|业界同构\|业界主流" "任务文档/04-TMT记忆系统.md"`
Expected: ≥3。
