...
- **LLM 客户端共享（模块 4 文档定稿，代码待实现）**：`services/llm.py` 模块级单例——`llm`（`init_chat_model` 原始实例，供 `build_graph()` 内 `bind_tools`）与 `call_llm(prompt)`（裸调用，供记忆层）。`graph.py` 与 `memory/store.py` 共用，`bind_tools` 留在 `build_graph()` 内（工具绑定是图特有的）。
