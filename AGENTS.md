...
backend/           uv 项目（src layout），全部后端代码
  src/agentic_search/
    ...
    memory/        空占位包（模块4 目标产物 memory.py 记忆加工 + store.py 记忆存储，文档已定稿，见 任务文档/04-TMT记忆系统.md）
  tests/           3 个测试文件（见下）
...
- **记忆层双文件（模块 4 文档定稿，代码待实现）**：`memory/memory.py`（记忆加工）= Memory dataclass + `extract_l1`(每轮自动)/`consolidate_l2`/`consolidate_profile`(按钮触发)/`get_memories_for_context`(配额注入取数)；`memory/store.py`（记忆存储）= `_client`/`_db`/私有 `_memories_collection`（对齐 `_documents_collection` 规范）+ `save_memory`/`load_memories`/`upsert_l2`/`upsert_profile`(幂等写入，返回 `_id`)。依赖方向单向：`memory.py → store.py → MongoDB`；`routes.py` 两者都调；图节点只调 `memory.py`。Memory 四字段 `{level, content, timestamp, session_id}`，L5 的 `session_id=None`（画像全局唯一，幂等键 `level="L5"`）；L2 幂等键 `{session_id, level}`。零向量依赖。
- **记忆端点契约（模块 4 文档定稿）**：L2/L5 共用 `POST /api/consolidate`（端点总数保持 4 个），请求体 `ConsolidateRequest{session_id, level="L2" 缺省}`，响应 `ConsolidateResponse{status, l2_id="", profile_id=""}`——纯增量扩展，端点零 Mongo 集合引用（加工调 memory.py、写入调 store.py）。图节点名 `retrieve_memory`（进入循环前，调 get_memories_for_context）与 `store_memory`（循环后，extract_l1），对齐 02 文档前向引用；`MemoryState(MessagesState)` 加 `session_id: str`；`QueryRequest` 加 `session_id: str = "default"`。
- **LLM 客户端共享（模块 4 文档定稿，代码待实现）**：`services/llm.py` 模块级单例——`llm`（`init_chat_model` 原始实例，供 `build_graph()` 内 `bind_tools`）与 `call_llm(prompt)`（裸调用，供记忆层）。`graph.py` 与 `memory/memory.py` 共用（store.py 零 LLM 依赖），`bind_tools` 留在 `build_graph()` 内（工具绑定是图特有的）。`call_llm` 里外层 `str(llm.invoke(...).content)` 是 Pylance 绕过（`content` 静态类型为联合类型，同 `parse_pdf` 惯例），别删；消费方解析用 `json.loads`（吃字符串）。
...
| `任务文档/04-TMT记忆系统.md` | 模块 4 定稿设计：三层 TMT 记忆（L1/L2/L5）、memory.py/store.py 双文件函数签名、prompts.yaml、`/api/consolidate` level 分流契约、前端三按钮、测试清单 |
...
4. **`memory/` 是空包**：模块 4 代码未实现，图当前无持久记忆/checkpointer（只有 LangGraph 临时 `MessagesState`）。**设计已定稿在 `任务文档/04-TMT记忆系统.md`**（三层 TMT + 双文件 memory.py/store.py + prompts.yaml + 配额注入），实现时严格按文档，尤其：`_memories_collection` 私有化收在 store.py、加工/存储勿混放、`retrieve_memory` 节点名（勿写成 get_memories）。
